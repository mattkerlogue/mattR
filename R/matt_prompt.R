...
#'
#' Add the following code to the .First() function in your .Rprofile file:
#'   ```mattR::matt_prompt()
#'   addTaskCallback(mattR::matt_prompt)```
#'
#' Positron does not properly support `.First()`, in which case use `init_prompt()`
#'
#' @export
matt_prompt <- function() {
  ...
  git_branch <- suppressWarnings(system(
  ...
  if (length(git_branch) != 0) {
    ...
    git_status <- suppressWarnings(system(
      "git status -s",
      ignore.stderr = TRUE,
      intern = TRUE
    ))
    git_ahead <- suppressWarnings(system(
      "git status -sb",
      ignore.stderr = TRUE,
      intern = TRUE
    ))
